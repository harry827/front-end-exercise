 ...
 */
// 提示：通常，事件名是驼峰命名 (camel-cased) 的字符串。不过也没有强制的要求，任何字符串都是可以使用的。
// 为了处理发出的事件，我们将函数 (Function) 关联到对象上。 我们把这些函数称为监听器 (listeners)。
...
// 类方法: EventEmitter.listenerCount(emitter, event) -- 返回指定事件的 listeners 个数
